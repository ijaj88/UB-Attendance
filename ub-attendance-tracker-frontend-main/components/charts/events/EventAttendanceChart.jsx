...
const EventAttendanceChart = ({eventId, eventFromDate}) => {
  // if(!eventId || !eventFromDate){
  //   return;
  // }
  ...
  useEffect(() => {
    ...
    setLoading(true)
    userService.getEventAttendanceWithId(eventId,fromDate,toDate).then(
      (output) =>{
  ...
  const renderTooltip = (props) => (
    <BootStrapTooltip id="overlay-tooltip" {...props}>
      For the selected Event this graph displays the User Attendance over Time, You could set filter for daily/weekly/monthly/yearly basis and observe the trends based upon users department, levels and affiliations.
    </BootStrapTooltip>
  );
